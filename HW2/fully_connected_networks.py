...
class Linear(object):
    ...
    @staticmethod
    def forward(x: torch.Tensor, w, b):
        """
    ...
    @staticmethod
    def backward(dout: torch.Tensor, cache):
        """
        ...
        dw = x_row.T @ dout
        db = (torch.ones((1,N), dtype= dout.dtype, device= dout.device) @ dout).squeeze()
        ##################################################
        #                END OF YOUR CODE                #
        ##################################################
        return dx, dw, db
...
def softmax_loss(x, y):
    ...
    # softmax = torch.exp(-x)
    # softmax_sum = softmax.sum(dim = 1)
    # softmax = (softmax.T / softmax_sum).T
    softmax_true = softmax[list(range(N)), y]
    # print(softmax_true.shape)
    loss = -torch.log(softmax_true).sum()/N
    dx = softmax.clone()/N
    dx[list(range(N)), y] = -(1 - softmax_true)/N
    ######################################################################
    #                END OF YOUR CODE                                    #
    ######################################################################
    return loss, dx
...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        #############################################################
        # Replace "pass" statement with your code
        out_fc1_relu, cache1 = Linear_ReLU.forward(X, self.params['W1'], self.params['b1'])
        scores, cache2 = Linear.forward(out_fc1_relu, self.params['W2'], self.params['b2'])
        ##############################################################
        #                     END OF YOUR CODE                       #
        ##############################################################
        ...
        ###################################################################
        # Replace "pass" statement with your code
        cross_entropy_loss, dout_fc2 = softmax_loss(scores, y)
        L2_loss = self.reg * (torch.norm(self.params['W1'])**2+ torch.norm(self.params['W2'])**2)
        loss = cross_entropy_loss + L2_loss
        dout_fc1_relu, grads['W2'], grads['b2'] = Linear.backward(dout_fc2, cache2)
        _, grads['W1'], grads['b1'] = Linear_ReLU.backward(dout_fc1_relu, cache1)
...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        for i in range(self.num_layers):
          if i==0: in_dim, out_dim = input_dim, hidden_dims[i]
          elif i==self.num_layers-1: in_dim, out_dim = hidden_dims[i-1], num_classes
          else: in_dim, out_dim = hidden_dims[i-1], hidden_dims[i]
    ...
    def loss(self, X: torch.Tensor, y=None):
        ...
        cache = []
        for i in range(self.num_layers):
          if i==0: scores = X.clone()
          if i== self.num_layers-1:
            scores, cache0 = Linear.forward(scores, self.params[f'W{i+1}'], self.params[f'b{i+1}'])
          else:
            scores, cache0 = Linear_ReLU.forward(scores, self.params[f'W{i+1}'], self.params[f'b{i+1}'])
            if self.use_dropout:
              scores, cache1 = Dropout.forward(scores, self.dropout_param)
              cache0 = (cache0, cache1)
          cache.append(cache0)
        #################################################################
        #                      END OF YOUR CODE                         #
        #################################################################
        ...
        #####################################################################
        # Replace "pass" statement with your code
        cross_entropy_loss, dout = softmax_loss(scores, y)
        L2_loss = self.reg/2 * torch.tensor([torch.norm(self.params[f'W{i+1}'])**2 for i in range(self.num_layers)]).sum()
        loss = cross_entropy_loss + L2_loss
        for i in range(self.num_layers-1, -1, -1):
          if i==self.num_layers-1:
            dout, dw, grads[f'b{i+1}'] = Linear.backward(dout, cache[i])
          else:
            cache0 = cache[i]
            if self.use_dropout:
              cache1 = cache0[1]
              cache0 = cache0[0]
              dout = Dropout.backward(dout, cache1)
            dout, dw, grads[f'b{i+1}'] = Linear_ReLU.backward(dout, cache0)
